...
describe('getRootMargin', () => {
  it('should return the correct root margin', () => {
    expect(getRootMargin({ offset: 0, direction: 'horizontal' })).toBe('0% 0px -100% 0px');
    expect(getRootMargin({ offset: 0.5, direction: 'horizontal' })).toBe('-50% 0px -50% 0px');
    expect(getRootMargin({ offset: 1, direction: 'horizontal' })).toBe('-100% 0px 0% 0px');
  });
});
